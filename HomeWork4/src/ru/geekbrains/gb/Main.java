...
public class Main {
    ...
    //Diagonals are universal, columns and rows are not(
    static boolean isWin(char[][] field, char sign) {
        for (int i = 0; i < field.length; i++) {
            if (field[i][i] == sign && field[i][1] == sign && field[i][2] == sign ||
                    field[0][i] == sign && field[1][i] == sign && field[2][i] == sign)
                return true;
        }
        int countDiagonal1 = 0, countDiagonal2 = 0;
        for (int i = 0; i < field.length; i++)
            for (int j = 0; j < field[i].length; j++) {
                if (i == j && field[i][j] == sign) countDiagonal1++;
                if (field[i][field.length - i - 1] == sign) countDiagonal2++;
            }
